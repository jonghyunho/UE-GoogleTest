...
class GivenAFloatingActor : public ::testing::Test {
 ...
 public:
  GivenAFloatingActor() : mFloatingActor(mMockActor, mMockMath) {
    ...
    ON_CALL(mMockActor, GetActorLocation())
        .WillByDefault(::testing::Return(mVector));
  };
};
...
TEST_F(GivenAFloatingActor, WhenFirstTickShouldNotMove) {
  ...
  EXPECT_CALL(mMockActor, SetActorLocation(::testing::Field(&FVector::Z, 0)))
      .Times(1);
...
TEST_F(GivenAFloatingActor, WhenTicksTwiceShouldMoveBySinDifferenceTimes20) {
  ...
  EXPECT_CALL(mMockActor,
              SetActorLocation(::testing::Field(&FVector::Z, 20.0f)))
      .Times(1);
...
TEST_F(GivenAFloatingActor,
       WhenTicksThreeTimesShouldConsiderDifferenceBetweenLast2Ticks) {
  EXPECT_CALL(mMockMath, Sin(0.0f)).WillRepeatedly(::testing::Return(0.0f));
  EXPECT_CALL(mMockMath, Sin(1.0f)).WillRepeatedly(::testing::Return(1.0f));
  ...
  EXPECT_CALL(mMockActor,
              SetActorLocation(::testing::Field(&FVector::Z, -10.0f)))
